...
public class TreeParser {
   ...
   * @return true when file is nexus format, false if nexus strings weren't
   *         found.
   */
            ...
            if (st.ttype == StreamTokenizer.TT_WORD
              ...
              while (st.ttype != StreamTokenizer.TT_EOF && !endOfTreeList) {
                ...
                if (st.ttype == StreamTokenizer.TT_WORD) {
                  ...
                  st.nextToken(); // eat a non-word while looking for first tree
                                  // word

                // System.out.println("Not a word while looking for a tree start tag: "
                // + st.ttype);
              }
            }
   ...
   *          the start of a newick file (basically the beginning of a newick
   *          tree, is run for each tree in a nexus file)
   */
  public TreeParser(BufferedReader b) {
    ...
    tokenizer.wordChars('\'', '\''); // quote problem, turn this into a prime
                                     // symbol?
    // 32 = space
    ...
    // 65-90 = [A-Z]
    // tokenizer.wordChars('[', '['); // 91 [ nexus comment character, treat as
    // char
  ...
  /**
   * Debug printout function. Avoid using the system calls and use this, and set
   * flag {@link #debugOutput} depending on debugging or not.
   * 
   * @param s
  ...
  /**
   * Adds node at the top of the stack to the tree. TreeNode is already created
   * based on Newick properties.
   * 
   * @param name
   *          Name of the node.
   * @param nodeStack
   *          Stack of nodes that haven't been added to the tree yet. Nodes are
   *          popped when they have names and all children are processed.
  ...
  public Tree tokenize() {
    ...
    try {
      while (EOT == false
        ...
        case StreamTokenizer.TT_WORD:
          if (tokenizer.sval.matches("[eE]-?[0-9]+")) {

            String numberToParse = tokenizer.sval;
            int exponent = Integer.parseInt(numberToParse.trim().substring(1));
            // update branch length
            if (lastNamed != null) {

              for (int ee = 0; ee > Math.abs(exponent); ee++) {
                if (exponent < 0) {
                  lastnum /= 10.0;
                } else {
                  lastnum *= 10.0;
        ...
        case StreamTokenizer.TT_NUMBER:
          if (nameNext)
            ...
            if (lastNamed != null)
              ...
              System.err.println("Error: can't set value " + tokenizer.nval
                  + " to a null node");
            lastNamed = null;
          }
          lastnum = new Double(tokenizer.nval);
          nameNext = false;
          break;
        case infoSeparator:
          if (nameNext)
    ...
    if (!nodeStack.isEmpty())
      System.err
          .println("Node stack still has " + nodeStack.size() + " things");
    t.postProcess();
    return t;
  }
   ...
   * 
   * @param treeNumbers
   *          Vector of Integers for commandline-based input of nexus trees;
   *          assume this vector is in ascending order
      ...
      try {
        while (!treeSectionEnd
          ...
          case commentOpen:
            ...
            while (thisToken != StreamTokenizer.TT_EOF
  ...
  /**
   * Tokenize a nexus file, uses newick tokenizer after identifying the region
   * with the tree information.
   * 
   * @param treeNumbers
   *          Vector of Integers for commandline-based input of nexus trees;
   *          assume this vector is in ascending order.
      ...
      while (EOF == false
        ...
        case StreamTokenizer.TT_WORD:
          if (tokenizer.sval.equalsIgnoreCase(nexusFileID))
            ...
            debugOutput("Word: " + tokenizer.sval);
          break;
        case commentOpen:
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Tree t = tp.tokenize();
      System.out.println("Tree is : " + t.drawTreeString(false, false));
    } catch (FileNotFoundException e) {
